...
def preprocess_excel(df: pd.DataFrame) -> pd.DataFrame | None:
    ...
    # 2) 우선 datetime으로 파싱 시도 (문자열/엑셀 시간 전부 커버)
    time_parsed = pd.to_datetime(df[TIME_COL], errors="coerce")

    if time_parsed.notna().any():
        # 적어도 일부는 datetime으로 인식됨 → 그걸 기준으로 hour 추출
        df["hour"] = time_parsed.dt.hour
    else:
        # 전혀 datetime으로 못 읽었을 때: 숫자 기반이라고 보고 정수 변환 시도
        df["hour"] = pd.to_numeric(df[TIME_COL], errors="coerce") % 24

    # hour에서 NaN인 행은 버린다
    df = df.dropna(subset=["hour"])
    if len(df) == 0:
        st.error("측정시각을 hour(0~23)로 변환할 수 있는 행이 없습니다.")
        return None
