...
TIME_COL = "측정시각"       # datetime 이거나 0~23 정수
VOLUME_COL = "평균교통량"
SPEED_COL = "평균속도"
CONG_COL = "혼잡빈도수"

# --- 두 지점 사이 거리 (km) : 나중에 네가 직접 채우면 됨 ---
#   실제로는 네이버 지도, 카카오맵, 구글 지도에서 구간 거리 찾아서 넣기
DISTANCE_MAP = {
    # ("구서IC", "영락IC"): 10.5,
    # ("영락IC", "부산TG"): 15.2,
    # ("부산TG", "노포IC"):  8.3,
...
def extract_nodes_from_segments(df):
    """
...
def build_graph_and_time_data(df):
    """
    엑셀에서 다음 정보를 만든다.
    - graph : {노드: [이웃노드 리스트]}
    ...
    for (seg, h), sub in grouped:
        ...
        C_e = sub[CONG_COL].sum()
        congestion_by_hour[(seg_clean, int(h))] = float(C_e)

        tp = sub[VOLUME_COL].sum()
        throughput_by_hour[(seg_clean, int(h))] = float(tp)

        speeds = sub[SPEED_COL]
        speeds = speeds[speeds > 0]
        if len(speeds) > 0:
            v = float(speeds.mean())
        else:
            v = DEFAULT_SPEED
        speed_by_hour[(seg_clean, int(h))] = v
...
def dijkstra_with_time(start, end, start_hour,
                       graph,
                       segment_to_nodes,
                       distance_for_segment,
                       congestion_by_hour,
...
def main():
    ...
    if len(nodes) < 2:
        st.error("인식된 지점이 너무 적습니다. 콘존명을 확인하세요.")
        return
